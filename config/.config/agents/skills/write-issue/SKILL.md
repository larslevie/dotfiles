...
After creating an issue, run the setup script to apply standard configuration. The script lives in the skill's base directory — use the path provided at the top of the skill context (e.g. `Base directory for this skill: /path/to/skill`):

```bash
/path/to/skill/setup-issue.sh <issue-url-or-number> [--type <type>] [--label <label>]... [--repo <owner/repo>]
```
...
3. Run `<skill-base-dir>/setup-issue.sh <url> --type <type>` to add label, set type, and add to project
